...
export class CommonHandlerService {
    ...
    private readonly logger = new Logger(CommonHandlerService.name);
    ...
    async assignTableSettings(params: any): Promise<any> {
        try {
            ...
            this.logger.error('Error in room.commonHandler-service.assignTableSettings', error.stack || error.message);

            let infoMessage = popupTextManager.dbQyeryInfo.DB_GETSPACTATOR_SETTING_FAIL;

            if (error.message?.includes("getCustomUser")) {
                infoMessage = popupTextManager.dbQyeryInfo.DB_GETUSERSETTINGS_FAIL;
            } else if (error.message?.includes("insertTableSetting")) {
