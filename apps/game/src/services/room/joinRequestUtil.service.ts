...
import { BroadcastHandlerService } from "./broadcastHandler";
...
export class JoinRequestUtilService {
    ...
    async joinPlayerToChannel(params: any): Promise<any> {
        ...
        const validated = await new Promise<any>((resolve) => {
            keyValidator.validateKeySets("Request", "connector", "joinPlayerToChannel", params, (res: any) => resolve(res));
        });
...
/*=============================  END  ========================*/
    ...
    async getTableDataForValidation(params: any): Promise<any> {
        ...
        try {
          const result = await this.db.findTableByIdAsync(params.channelId); // Assumes `db.findTableByIdAsync` exists or is created
