...
export class ResponseHandlerRoomService {
    ...
    private readonly logger = new Logger(ResponseHandlerRoomService.name);
    ...
    calculateAddonTimeRemaining(params: any): number {
        ...
        try {
            ...
            this.logger.error('Error in room.responseHandlerRoom-service.calculateAddonTimeRemaining', error.stack);
            throw new Error(`Failed in room.responseHandlerRoom-service.similarTableBroadcast: ${error.message}`);
        }
    ...
    async setJoinChannelKeys(params) {
        ...
        try {
            ...
            if (params.table.channelType === stateOfX.gameType.tournament) {
                ...
                let nextBlindData = this.setNextBlindInfoForJoinChannelKeys(params.table);
            ...
            this.logger.error('Error in room.responseHandlerRoom.setJoinChannelKeys', error.stack);
            throw new Error(`Failed in room.responseHandlerRoom.setJoinChannelKeys: ${error.message}`);
        }
