...
@Injectable()
export class HandInHandManagementService {
    ...
    async getAllChannels(params: any): Promise<any> {
        ...
        try {
            ...
            params.channels = channels.filter(channel => channel.players.length > 0);
    ...
    async isEligibleForHadInHand(params: any): Promise<any> {
        ...
        console.error("error in players length", err);
        return { success: false, info: "Error determining eligibility" };
        }
    }
    ...
    async isTimeToStartBreakTimer(params: any): Promise<any> {
        ...
        params.allChannels = params.channels.map((channel) => channel.channelId);
    ...
    async process(params: any): Promise<any> {
        try {
        const channelsResult = await this.getAllChannels(params);
        const eligibleResult = await this.isEligibleForHadInHand(channelsResult);
        const breakTimerResult = await this.isTimeToStartBreakTimer(eligibleResult);
        const updatedParams = await this.updateBreakTimer(breakTimerResult);
        updatedParams.success = true;
        return updatedParams;
        } catch (err) {
        console.error("Error in handInHandManagement.process", err);
        throw err;
        }
    };
