...
async startRITProcess(params) {
  ...
  const moveData = await this.moveDataMap.get(params.channelId);
  moveData.data.evChopDetails = params.data.evChopDetails;
  const calledAfterRIT = params.calledFromChannelHandler || false;

  params = moveData;
  params.calledAfterRIT = calledAfterRIT;

  if (!!params.data.evChopDetails && !!params.table.isRunItTwiceTable) {
    const finalResponse = await this.handleGameOver.afterProcess(params);
  ...
  if (!!params.data.evChopDetails && !params.table.isRunItTwiceTable && params.data.evChopDetails.length === 2) {
    ...
    console.log("allPlayers evChop value in RITProcess is", allEvChopFalse);

    if (allEvChopFalse) {
      params.data.evChopDetails.sort((a, b) => b.equity - a.equity);
      let maxEquityPlayer = params.data.evChopDetails[0];
      let otherPlayer = params.data.evChopDetails[1];

      console.log("maximum equity player is", maxEquityPlayer, otherPlayer);

      const restPlayers = params.table.players.filter(player => player.playerId !== maxEquityPlayer.playerId).map(player => ({ playerId: player.playerId, cards: player.cards }));
      let outCards: any[] = [];
      let outCount = 0;
      ...
      if (params.calledAfterRIT) {
        ...
        if (params.data.roundName !== 'PREFLOP') {
          const outsData = await this.outsScript.getOuts(params.data.evChopDetails, params.table.boardCard[0], params.table.channelVariation, params.table.deck);
      ...
      const ritData = {
        ...
        outs: { count: outCount, cards: outCards },
      ...
      await new Promise<void>((resolve) => {
        pomelo.app.rpc.room.roomRemote.evRIT(pomelo, ritData, async (_err: any, _res: any) => {
          imdb.EvChopDetails(params.channelId, { ritDetails: ritData }, (_err, _updatedTable) => {
            channel.evRITTimer = setTimeout(async () => {
              imdb.getTable(params.channelId, async (_err, table) => {

                for (const player of params.data.evChopDetails) {
                  const foundIndex = table.players.findIndex(x => x.playerId === player.playerId);
                  player.evRIT = !!table.players[foundIndex]?.evRIT;
                }
                const finalResponse = await this.afterProcess(params);
                resolve();
              });
            }, (params.table.ritPopupTime + 1) * 1000);
            channel.evRITTimer.startedAt = Number(new Date());
          });
        });
      });
      return this.afterProcess(params);
    }
  }
...
var awardWinningChips = function (params, cb) {
    ...
    if (params.data.evChopDetails) {
        ...
        for (let data of params.data.evChopDetails) {
            ...
            data.evAmount = convert.convert(data.evAmount - rakeAmount * (data.equity / 100));
        }
    }

    console.log("inside awardWinningChips", params)
    console.log("inside awardWinningChips evChopDetails", params.data.evChopDetails)
    console.log("inside awardWinningChips params.data", params.data)
    console.log("inside awardWinningChips params.data.winners", params.data.winners)
    console.log("inside awardWinningChips params.data.contributors", params.data.contributors)
    console.log("inside awardWinningChips rakeDetails", params.rakeDetails)
    console.log("inside awardWinningChips winners", params.winners)
    console.log("inside awardWinningChips params.data decisionParams", params.data.decisionParams)
    console.log("inside awardWinningChips", params.table)
    console.log("inside awardWinningChips table.pot is", params.table.pot)
        ...
        if (isGameProgressResponse.success) {
            winnerRemote.awardWinningChips(params, function (err, awardWinningChipsResponse) {
                ...
                if (!!params.data.evChopDetails) {
                    ...
                    for (let player of params.data.evChopDetails) {
                        if (player.evChop) {
                            // tiePotAmount += player.potAmount;
                            let indexes = params.data.winners.reduce((acc, curr, index) => {
                                if (curr.playerId === player.playerId) {
                                    acc.push(index);
                                }
                                return acc;
                            }, []);
                            console.log("indexInWinners is index pe", indexes)
                            console.log("indexInWinners is index pe", indexes.length)
                            if (indexes.length) {
                                for (let index of indexes) {
                                    console.log("picking first index", index)
                                    let percentage = (player.evAmount / player.potAmount);
                                    if (params.data.winners[index].isRefund && player.refundAmount) {
                                        params.data.winners[index].winningAmount = convert.convert(params.data.winners[index].amount * percentage + player.refundAmount);
                                    }
                                    else {
                                        params.data.winners[index].winningAmount = convert.convert(params.data.winners[index].amount * percentage);
                                    }
                                }
                            }
                            let foundIndex = params.table.players.findIndex(x => x.playerId == player.playerId);
                            let playerName = params.table.players[foundIndex].playerName;
                            let query = {
                            ...
                            logDB.evHistory(query, data, function (err, response) {
                                console.log("inside evHistory logdb update", err, response)
                            })
                        }
                    }
                    console.log("params.data.winners after editing is", params.data.winners)
                    console.log("saving admin tie data", params)
                    // if(params.totalTie){
                    //   let query = {
                    //     handId: params.table.roundNumber,
                    //     userName: 'admin'
                    ...
                    //     createdAt: Number(new Date()),
                    //     potAmount: 0,
                    //     profit: convert.convert((params.totalTie/100) * 0),
                    //   }
                    //   logDB.evHistory(query, data, function (err, response){
                    //     console.log("inside evHistory logdb update", err, response)
                    //   })
...
var assignPlayercards = function (params, cb) {
    ...
    if (params.data.isSingleWinner) {
        ...
        if (playerIndexOnTable >= 0) {
            ...
            imdb.findTableSetting({ playerId: player.playerId, channelId: params.channelId }, function (err, result) {
                if (!err) {
                    ...
                    if (!isMuckHand) {
                        ...
                        var moveData = {};
                        moveData.cards = player.cards;
                        moveData.lastMove = player.lastMove;
                        moveData.playerId = player.playerId;
                        moveData.name = player.playerName;
...
var createGameOverResponse = function (params, cb) {
    ////console.log("23232323 createGameOverResponse", params)
...
handleGameOver.afterProcess = (params) => {
    ...
    console.log("find channel in processGameOver afterProcess", channel)
    if (!!channel && channel.evRITTimer) {
        clearTimeout(channel.evRITTimer)
        channel.evRITTimer = null;
    }
    if (!!channel && channel.evChopTimer) {
        clearTimeout(channel.evChopTimer)
        channel.evChopTimer = null;
    }
    let moveData = moveDataMap.get(params.channelId);

    console.log("params before afterProcess", params, moveData)
    moveData.data.evChopDetails = params.data.evChopDetails;
    moveData.table = params.table;
    params = moveData;
    console.log("params after afterProcess", params)

    return new Promise((resolve, reject) => {
        imdb.getTable(params.channelId, async function (err, table) {
            console.log("imdb getTable in afterProcess", err, table)
            if (!err && table && !params.doNotCallActionHandler) {
                params.table = table
            }
            async.waterfall([
                async.apply(assignPotIndexAndBoard, params),
                ...
                if (err) {
                    console.log("console in handleGameOver res after awarding chips is", err, response);
                    ...
                    console.log("response inside afterProcess waterfall is", err, response);
                    if (!!response.params.data) {
                        ...
                        let tableValue = _.omit(response.params.table, '_id');
                        console.log("tableValue is", tableValue)
                        mongodb.inMemoryDb.collection("tables").findAndModify(
                            { channelId: params.channelId },
                            [],
                            { $set: tableValue },
                            { upsert: false, new: true }, function (err, table) {
                                console.log("after updating res is", err, table)
                            }
                        );
                        ...
                        if (!params.doNotCallActionHandler) {
                            responseHandler.setActionKeys(response.params, function (setActionKeysResponse) {
                                console.log("setActionKeys is", setActionKeysResponse);
                                console.log("final response after setActionKeys is", response.params)
                                pomelo.app.rpc.room.roomRemote.handle(pomelo, { session: {}, channelId: setActionKeysResponse.data.response.channelId, response: setActionKeysResponse.data.response, request: requestMade, alreadySentTurn: true }, function (err, res) {
                                    console.log("actionHandler handle response is", err, res)
                                });
                            });
                        }
                    }
...
handleGameOver.processGameOver = function (params, cb) {
    ...
    console.trace("23232323 in handleGameOver params", params)
    console.log("23232323 in handleGameOver data", params.data)
    console.log("23232323 in handleGameOver table", params.table)
    console.log("23232323 in handleGameOver table players", params.table.allInPLayerCardsCards)
    console.log("23232323 in handleGameOver", params.table.deck)

    serverLog(stateOfX.serverLogType.info, 'In handleGameOver function processGameOver');
    serverLog(stateOfX.serverLogType.info, "=========== GAME OVER CALCULATION STARTED ===========")
    keyValidator.validateKeySets("Request", "database", "processGameOver", params, function (validated) {
        if (validated.success) {
            async.waterfall([
                ...
                console.log("console in handleGameOver first res is", err, response)
                console.log("console in handleGameOver first waterfall data", params.data)
                console.log("console in handleGameOver first waterfall table", params.table)
                if (err) {
                    cb(err);
                } else {
                    moveDataMap.set(response.params.channelId, response.params);
                    if (!!params.table.isEvChopTable && params.data.roundName !== stateOfX.round.river && params.data.roundName !== stateOfX.round.showdown && params.table.allInPLayerCardsCards.length >= 2) {
                        console.log("inside CHECKING TO start ev process params.data decisionParams are", params.data.decisionParams)
                        console.log("inside CHECKING TO start ev process params.data decisionParams are", params.table.pot)
                        ...
                        if (finalConsiderPot >= 10 * params.table.bigBlind) {
                            ...
                            var nextParams = JSON.parse(JSON.stringify(response.params));
                            console.log("giving back gameOver in if response is before", response.params.data)

                            console.log("last move data is", moveDataMap.get(response.params.channelId));
                            console.log("giving back gameOver in if response is", response, response.params.data)
                            cb(response);

                            if (response.params.data.isCurrentPlayer) {
                                let refundPot = _.filter(response.params.table.pot, (item) => item.contributors.length == 1);
                                let turnData = {
                                    success: true,
                                    ...
                                    amount: response.params.data.amount,
                                    action: (response.params.table.channelType === stateOfX.gameType.tournament && (response.params.data.action === stateOfX.move.standup || response.params.data.action === stateOfX.move.leave)) ? stateOfX.move.fold : response.params.data.action,
                                    ...
                                    moves: [],
                                    totalRoundBet: 0,
                                    lastPlayerBet: (response.params.data.action === stateOfX.move.standup || response.params.data.action === stateOfX.move.leave ? 0 : response.params.data.considerAmount),
                                    ...
                                    totalPot: tableManager.getTotalPot(_.filter(response.params.table.pot, (item) => item.contributors.length > 1)),
                                ...
                                imdb.EvChopDetails(response.params.channelId, { turnEvData: turnData }, function (err, updatedTable) {
                                    console.log("imdb after updating turnData", err, updatedTable);
                                    pomelo.app.rpc.room.roomRemote.turn(pomelo, turnData, function (err, res) {
                                        console.log("roomRemote turn response is", err, res)
                                    });
                                })
                            }
                            let roundOverData = {
                            ...
                            console.log("now nextParams is", nextParams)
                            // Fire round over broadcast
                            pomelo.app.rpc.room.roomRemote.roundOver(pomelo, roundOverData, async function (err, res) {
                                console.log("roomRemote roundOver response is", err, res)
                                let fireAllInCards = {
                                    channelId: nextParams.channelId,
                                    allInCards: nextParams.data.allInCards,
                                };
                                pomelo.app.rpc.room.roomRemote.fireAllInCards(pomelo, fireAllInCards, async function (err, res) {
                                    console.log("roomRemote fireAllInCards response is", err, res)
                                    let res1 = await startEVProcessNew(nextParams);
                                    console.log("evProcess response after 10 seconds is", res1)
                                    let res2 = await handleGameOver.startRITProcess(res1);
                                    console.log("ritProcess response after 5 seconds is", res2)
                                    cb(res2);
                                });
                            });
                        }
                            ...
                            let res = await handleGameOver.afterProcess(response.params);
                            console.log("giving back gameOver in inside else response is", res)
                            cb(res);
                        }
                    }
                    else {
                        response.params.doNotCallActionHandler = true;
                        let res = await handleGameOver.afterProcess(response.params);
                        console.log("giving back gameOver in else response is", res)
                        cb(res);
                    }
                }
            })
