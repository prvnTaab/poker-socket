...
export class DeckService {
    ...
    private makeCards(): void {
        for (const type in this.types) {
            for (let rank = 1; rank <= 13; rank++) {
                this.deck.push(new CardsService(type, rank));
            }
        }
    }

    public getCards(): CardsService[] {
        return this.deck;
    }
    ...
    public shuffle(): void {
        ...
        while (len !== 0) {
            const randIdx:any = Math.floor(Math.random() * len);
            len--;
    ...
    public getRandomCards(num: number): CardsService[] {
        const randCards: CardsService[] = [];
