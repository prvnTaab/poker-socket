...
export class UtilityService {
  ...
  private readonly url = new URL('https://control.msg91.com/api/v5/email/send');
   ...
   */
  dispatcher<T>(key: string, list: T[]): T {
  ...
  /**
   * Extracts specified keys from each object in a collection
   * @param collectionArray The array of objects
   * @param keysArray The keys to extract from each object
   * @returns A new array with objects containing only the specified keys
   */
  pluckKeys(collectionArray: any[], keysArray: string[]): any[] | { error: string } {
    const result: any[] = [];

    if (collectionArray?.length && keysArray?.length) {
      for (let i = 0; i < collectionArray.length; i++) {
        const tempObject: any = {};
        for (let j = 0; j < keysArray.length; j++) {
          tempObject[keysArray[j]] = collectionArray[i][keysArray[j]];
        }
  ...
  // Optional: Example method to send email using the configured URL
  ...
  // Optional: Add crypto encryption/decryption logic if needed here
  ...
  // keysArray - keys to be adde 
  sumKeys(collectionArray: any[], keysArray: string[]): any {
    const result: any = {};
  ...
  changeKeys(collectionArray: any[], object: any): any {
      ...
      for (const keys in json[obj]) { // first level keys
        for (const key in json[obj][keys]) { // second level keys
          for (const k in json[obj][keys][key]) { // third level keys
            this.convert(k, json[obj][keys][key]);
          }
          this.convert(key, json[obj][keys]);
        }
        this.convert(keys, json[obj]);
      }
    }
    return json;
  }
  ...
  decrypt(password: any) {
    try {
      var decipher = crypto.createDecipher(this.algorithm, this.privateKey);
  ...
  async sendMailWithHtml(data: any): Promise<any> {
      ...
      recipients: [
        {
          ...
          variables: this.getSubstitutions(data),
        },
      ],
      from: {
